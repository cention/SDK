...
const ChatModal = ({workSpace, widgetId}) => {
  ...
  const [areaFieldVisible, setAreaFieldVisible] = useState(false);
  ...
  const fetchChatSdk = async () => {
    ...
    if (data.areaId.length > 1) {
      setAreaFieldVisible(true);
    }
    setAreas(data.areaId);
    setAreaId(data.areaId[0].Id);
    const fetchAgents = await getActiveAgents(workSpace, areaId);
  ...
  const errorEmail = emailErrorText
    ? emailErrorText
    : 'Please enter a valid email address.';
  const questionInput = questionText ? questionText : 'Question...';
  const EulaLabel = eulaT
    ? eulaT
    : 'I consent to that information about this chat will be collected and stored in accordance with Data Privacy laws and directives.';
  const titleText = headerText ? headerText : 'Live chat';

  //-----------------------------------------------------------------------------------

  const opacity =
    showOtherContent || animationCompleted ? 1 : animationProgress / 70;
  ...
  return (
    <>
      <TouchableOpacity style={styles.floatingButton} onPress={toggleChatModal}>
                    ...
                    {showOtherContent && (
                      <View
                        style={[
                          ...
                          {marginTop: keyboardVisible ? '25%' : '-25%'},
                        ]}>
                        {emailError ? (
                          <Text
                            style={[styles.errorText, {color: warningColor}]}>
                            {emailError}
                          </Text>
                        ) : null}
                        {areaFieldVisible && (
                        ...
                        {eula && !keyboardVisible && (
                          <View style={styles.eulaTextContainer}>
                            <Text style={[styles.eulaText, {color: EULAColor}]}>
                              {EulaLabel}
                            </Text>
                          </View>
                        )}
...
const styles = StyleSheet.create({
  ...
  dropdownContainer: {
    ...
    width: '80%', // Match the width of your input or adjust as needed
